...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode = 'signin' }) => {
  ...
  // Prevent close button from flashing in center before Clerk form mounts
  ...
  useEffect(() => {
    if (isOpen) {
      setMode(initialMode);
      // Slight delay so the auth form mounts before showing the close button
      const timer = window.setTimeout(() => setShowCloseButton(true), 150);
      return () => {
